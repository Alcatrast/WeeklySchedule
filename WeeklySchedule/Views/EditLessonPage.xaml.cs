...
public partial class EditLessonPage : ContentPage
{
        ...
        {
            {
                ...
                if (Application.Current?.Windows[0]?.Page is Page page)
                    await page.DisplayAlertAsync("Ошибка", $"Не удалось сохранить пару: {ex.Message}", "ОК");
            }
        }
    }
    ...
    private async void OnDeleteClicked(object? sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            SetButtonsEnabled(true);
            if (Application.Current?.Windows[0]?.Page is Page page)
                await page.DisplayAlertAsync("Ошибка", $"Не удалось удалить пару: {ex.Message}", "ОК");
        }
    }
    ...
    private async void OnCancelClicked(object? sender, EventArgs e)
    {
        ...
        {
            if (Application.Current?.Windows[0]?.Page is Page page)
                await page.DisplayAlertAsync("Ошибка", $"Не удалось закрыть страницу: {ex.Message}", "ОК");
        }
    }
